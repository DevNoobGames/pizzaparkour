    ...
    public void LateUpdate()
    {
        ...
        if (CanAttach())
        {
            ...
            for (int i = 0; i < directions.Length; i++)
            {
                ...
                if (hits[i].collider != null && hits[i].collider.gameObject.CompareTag("Wall"))
                {
            ...
            if (CanWallRun())
            {
                hits = hits.ToList().Where(h => h.collider != null && h.collider.gameObject.CompareTag("Wall")).OrderBy(h => h.distance).ToArray();
                if (hits.Length > 0)
                {
                    OnWall(hits[0]);
